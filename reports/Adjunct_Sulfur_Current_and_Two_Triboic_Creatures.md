...
The practical bottom line, stated up front for anyone deciding where to spend effort: these two creatures share a metabolism and an architecture but sit at opposite ends of readiness. The Venus creature is gated on roughly a decade of wet-lab chemistry that begins with one keystone measurement. The vent creature is gated, in principle, on a single well-aimed dive. That contrast is the document's main output.
...
The electron-transfer machinery is the same extracellular electron transfer (EET) studied in the canonical electrogens. *Geobacter sulfurreducens* respires an electrode as its sole electron acceptor [LAB, Bond and Lovley 2003]. Directly relevant to the sulfur case, EET has been characterized in a dissimilatory sulfate-reducing bacterium, with both direct and indirect (shuttle-mediated) routes reported to operate during electroactive respiration [LAB, Hou et al. 2024]. This is the terrestrial anchor most important to the vent creature, because it is ordinary anoxic-water sulfur chemistry doing exactly the electron export the vent design assumes. *(The specific species, strain, and mechanistic detail of the Hou 2024 result are flagged for verification against the paper before publication; see Sources. Do not assert the species or the two named routes until confirmed.)*
...
- **The optima do not generalize.** Across these studies the best operating potential ranges widely with organism and system, and in the acetate study the potential advantage disappeared by the end of the run. "Applied potential tunes the metabolism" is [LAB]. "The optimum is a specific voltage" is not transferable. *(This section deliberately avoids quoting the specific best-performing potentials from each study. The original adjunct flagged those exact numbers as the thing not to trust across systems, and re-stating them here, even as examples, invites the reader to generalize the very figures the section says are non-generalizable. Any specific potential cited elsewhere should be verified against its paper and presented only as system-specific.)*
...
A triboelectric boost is a way to pay for the complexity an oxygen-free environment cannot otherwise fund. That is the entire point of the creature, so the metabolism has to be specified without any oxygen at all. The single-celled chemoautotrophs that live in the pure anoxic upflow do not use oxygen; they run anaerobic sulfur chemistry, most plausibly the reduction of sulfur oxides or the disproportionation of intermediate-oxidation-state sulfur, closing their circuit on an oxidant already present in the reducing fluid (CO₂-derived species, oxidized metals, or a higher sulfur oxidation state) rather than on O₂. The vent creature is proposed to run that same oxygen-free sulfur chemistry, with a triboelectric current supplement paying for the energetic step those single cells cannot afford: becoming multicellular.

The prediction is therefore a **multicellular organism living in the anoxic vent upflow, deeper than the oxygen boundary should allow**, anchored to rock, its whips fluttered by the upwelling, running an anaerobic sulfur metabolism that on Earth supports only single cells. The signature is multicellularity where the map says there should be none. If found, it is the single clearest demonstration of the project's central claim, that triboic current can let complex life be complex without oxygen. Specifying which oxygen-free sulfur couple it runs, and what closes the circuit, is the first item on its to-do list, and it is the direct counterpart of the Venus creature's keystone.

One objection belongs here, raised before a reader raises it. Earth has a fully populated tree of life and no known multicellular anaerobe of this kind, so the fair question is why evolution has never produced this creature if it is possible. The honest answer is that it may not have, and a clean null result is a real outcome of the survey below. But the project's origin tension cuts the other way too: a triboic organism that must be multicellular from the start (Report B, V-5) has no obvious single-celled ancestor, so if this creature exists it likely did not evolve by the usual incremental route, which is one reason it could be both real and never previously found. The creature carries that origin question rather than resolving it.
...
4. **Culture a candidate sulfur EET organism from the anoxic flow.** *Work: anaerobic microbiology.* Attempt to culture sulfur-cycling organisms from the upflow and test EET to an electrode, extending the *Desulfovibrio* result into vent isolates. The chemistry the foundation already makes plausible, but the culturing itself may be the hard part: a large fraction of vent microbes resist cultivation, so this item may require culture-independent methods (metagenomics, single-cell genomics) to identify the EET machinery even if isolation fails.
...
Two sulfur-powered triboic creatures share one demonstrated foundation and then diverge about as sharply as two organisms running the same metabolism could.
...
- Hou et al. (2024). Co-occurrence of direct and indirect extracellular electron transfer mechanisms during electroactive respiration in a dissimilatory sulfate-reducing bacterium. *Microbiology Spectrum*. **[citation needs verification]** on three points before publication: (i) the author list and first-author initial, (ii) the exact volume/issue/article-ID and DOI, and (iii) whether the paper names the specific species/strain and the two EET routes attributed to it in A1. Report B's 2nd draft cites a "Hou, Y. et al. 2024" at PMC11705803 for the same underlying result; confirm whether these are the same paper and make the citation identical across both documents. This is the load-bearing terrestrial anchor for the vent creature, so it must be exact.
