...
**Tier.** [SIM] for the ledger. [SPEC] for the bounded inputs: turnover rate, drag coefficient, gel leakage, yield strength. [LAB] for the microbial-energetics anchor underneath the chemistry term [18]. [LAB] for the engineered-TENG durability benchmarks [19–22]. [SPEC-CHOICE] for treating whip life as a repair-rate balance rather than a fixed fatigue life.
...
**The benchmark, and why it does not settle the question.** The turnover corners can be read against the engineered triboelectric literature, which is the only body of work that has cycled contact-electrification surfaces to failure. Converting at the binding 0.1–1 Hz flutter band (II-6), 0.3%/day turnover is a whip life of ~330 days, or ~8.6×10⁶ cycles at 0.3 Hz; the pessimistic 3%/day is ~8.6×10⁵ cycles. Against published devices: a fluttering wind-driven TENG holds 95% output to 970,000 cycles [19], but only with an engineered nano-oil lubricant barrier maintained at the contact; a diamond-like-carbon-on-glass sliding pair clears over 500,000 cycles by virtue of extreme hardness and low friction [20]; unhardened polymer and biological-material devices fail far earlier. The favourable corner therefore sits roughly an order of magnitude beyond the best hardened inorganic device ever reported, and the pessimistic corner sits near the engineered state of the art for a lubricated device. Read naively, this is damning: the corner the ledger needs is one no engineered TENG has reached.

That reading is wrong, and the reason it is wrong is the substantive finding. Every device in that comparison is a dead artifact whose surface is machined once and thereafter only degrades; its cycle count is a pure wear-accumulation limit. A tribowhip is not that. It is repaired continuously, and the ledger already pays for it: the ribbon-repair term, 8.9×10⁻¹³ W, is the largest single overhead in the budget. The relevant quantity is therefore not whether a polymer survives 8.6×10⁶ cycles unattended — nothing biological is being asked to — but whether the resurfacing rate the ledger has already bought exceeds the wear rate per cycle at the contact. Turnover is not a material constant to be looked up. It is the *output* of a race between repair and wear, and the ledger currently treats it as an input. This is the deepest structural weakness in the growth budget, and it was found by benchmarking rather than by introspection.

The engineered literature independently supports the reframing. The two longest-lived devices above do not achieve their cycle counts by hardness alone but by maintaining a lubricant layer at the contact, and at least one is explicitly biomimetic — modelled on the earthworm's continuous mucus secretion from epidermal glands [22]. Where engineering has approached the cycle counts this ledger needs, it has done so by copying active biological resurfacing. That is a convergence worth stating plainly, and it also exposes a design gap: Fulgorax has no resurfacing organ. Its whips are repaired by replacement, not maintained by secretion. Whether a secreting whip is a cheaper solution than a replaced one is unmodelled and is the most promising unexplored direction in the creature's design.

**Honest limit.** Every term is a lumped numpy estimate on [SPEC]-bounded inputs. The verdict is a bounded knife-edge with its flip-condition, durability, explicit, not a clean pass. What keeps the overheads from killing growth is the creature's passive design: drifting anchors do no work, and passive phase-stagger needs no conditioning circuit. So the result is contingent on that morphology being real. The benchmark above narrows nothing numerically: the engineered devices are dry-contact, room-temperature, unlubricated-or-oil-lubricated films in air, not sporopollenin-coated ribbons in supercritical-adjacent CO₂ under acid and dust loading, and no published device reports a wear rate per cycle in a form this ledger could consume. The comparison establishes a scale, not a value. **Where a critic should push first:** not the turnover number itself, which is [SPEC] and admitted, but the prior question of whether turnover is an input at all. If whip life is set by a repair-versus-wear balance, then a ledger that takes turnover as given and charges repair separately may be double-counting one side of that balance or omitting its coupling to flutter rate entirely. That is a structural objection, and it would not be answered by measuring the number better.
...
**Honest limit.** Every leg rests on the same class of input, fatigue life, turnover rate, wear, transferred from terrestrial and lab analogues and unmeasured in acid exposure under Venus dust loading. The convergence makes the structure of the conclusion robust, meaning which quantity binds, but not the absolute durability number. There is a mild circularity risk worth naming: three of the four legs feed into or draw from the same durability assumption, so the independence is in the physics, not always in the inputs. II-5 sharpens this: if whip turnover is an *output* of a repair-versus-wear balance rather than an input, then the shared quantity binding these legs may not merely be shared but mis-typed — treated as a fixed material constant when it is actually a rate set by the creature's own repair budget. That would not weaken the convergence on *which* quantity binds, but it would mean the binding quantity is downstream of the ledger, not independent of it. **Where a critic should push first:** whether the four legs are as independent as claimed, or whether a single shared turnover assumption — possibly itself an output rather than an input — is doing the work in more than one of them.
...
**D.8, Engineered-TENG durability benchmarks [LAB].** These are device-engineering results, cited in II-5 to establish the *scale* of achievable contact-electrification cycle life, not to transfer a value into the Venus regime. All are dry or oil-lubricated films at room temperature; none is regime-relevant, and the comparison in II-5 states this explicitly.

[19] Heo, D., et al. (2025). "Nano-Oil-Barrier-Based Fluttering Triboelectric Nanogenerator." *Advanced Science* 11, 2502278. DOI 10.1002/advs.202502278; PMC12376523. Wind-driven fluttering-mode device retaining 95% output after 970,000 cycles via a maintained nano-oil lubricant barrier. The closest published geometry to the tribowhip (flutter-driven), and the source of the "lubrication-maintained, not hardness-maintained" observation.

[20] Li, S., et al. (2024). "Durable and High-Performance Triboelectric Nanogenerator Based on an Inorganic Triboelectric Pair of Diamond-Like-Carbon and Glass." *Advanced Science* 11, 2309170. DOI 10.1002/advs.202309170; PMC11434241. Sliding-mode DLC–glass pair with stable output over 500,000 cycles by extreme hardness and low friction. The hardened-inorganic durability ceiling referenced in II-5.

[21] Zhai, L., Wen, H., Liu, H., Guo, D., Liu, G., Qu, H., Wan, L., Yao, H., Zhai, J. (2024). Multilayer contact-separation-mode stacked TENG. *Nano Energy* 125, 109546. Twelve-layer stack producing 152.16 μA, a 6.35× short-circuit-current increase over a single unit, the empirical basis for the *sublinear* stacking-scaling claim in II-5 and in the Frontier's stacked-whip question.

[22] Liu, M., Zhang, X., Xin, Y., Guo, D., Hu, G., Ma, Y., Yu, B., Huang, T., Ji, C., Zhu, M., Yu, H. (2024). "Earthworm-Inspired Ultra-Durable Sliding Triboelectric Nanogenerator with Bionic Self-Replenishing Lubricating Property." *Advanced Science* 11, 2401636. DOI 10.1002/advs.202401636; PMC11267296. A porous film that stores and releases lubricant under cyclic stress (1% attenuation over 100,000 cycles), biomimicking earthworm mucus secretion. Cited in II-5 as the engineered instance of *active resurfacing* — the design principle Fulgorax's replace-don't-maintain whip lacks.
